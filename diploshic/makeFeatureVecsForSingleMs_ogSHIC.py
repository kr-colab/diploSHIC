...
import diploshic.fvTools as fvTools
import time
    ...
    if len(unmaskedSnpIndices) == 0:
        ...
        for subWinIndex in range(numSubWins):
            ...
            snpIndicesInSubWinUnmasked = [
                x
                for x in snpIndicesInSubWins[subWinIndex]
                if unmasked[positionArray[x] - 1]
            ]  # NOQA
...
if numInstancesDone != numInstances:
    sys.exit(
        ...
        % (numInstancesDone, numInstances)  # NOQA
    )
